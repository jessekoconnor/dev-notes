...
test('basic copy test', () => {
    // Create: 1 -> 2, where 1.random -> 2 and 2.random -> 1
    ...
    assertEquals(copy !== node1, true);
    assertEquals(copy.val, 1);
    assertEquals(copy.next.val, 2);
    assertEquals(copy.random.val, 2);
    assertEquals(copy.next.random.val, 1);
});

test('null head', () => {
    assertEquals(copyRandomList(null), null);
});
